...
import xlrd 
from EarningsData import PricesAndDates #@UnresolvedImport
...
#         print(i)
    ...
    """Now get prices. Earnings date is end of fiscal year, will estimate date earnings came out
    by simply adding two months."""
    dates1 = dates[:]
    
    for i in range(1, len(dates1)):
        split = dates1[i].split("/")
        month = int(split[1]) + 1
        year = int(split[0])
        if(month > 12):
            month -= 12
        ...
        dates1[i] = str(year) + "/" + str(month) + "/" + split[2]

    prices = PricesAndDates.getHistoricalPrices(tickerName, dates1)
